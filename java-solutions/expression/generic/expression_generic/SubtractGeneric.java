...
public class SubtractGeneric<T> extends ActionsGeneric<T> {
    private final Generics<T> gen;
    public SubtractGeneric(Generics<T> gen, UnificationGeneric<T> left_expression, UnificationGeneric<T> right_expression){
        super(left_expression, right_expression);
        this.gen = gen;
    }

    public T binaryOperation(T left_result, T right_result){
        return  gen.subtract(left_result, right_result);
    }
